...
try:
    fire_data_df = pd.read_csv("yangin_ve_hava_verisi.csv")
    fire_data_df = fire_data_df.rename(columns={'latitude': 'lat', 'longitude': 'lon'})
except FileNotFoundError:
    st.error("Veri dosyası 'yangin_ve_hava_verisi.csv' bulunamadı. Lütfen dosya yolunu kontrol edin.")
    st.stop()
# ------------------------------
# 2️⃣ Hava Durumu Fonksiyonları
# ------------------------------
API_KEY = "e2cc91b090f4fdecb8b0aea827458fc6"  # çalıştığı doğrulandı
...
# Hava durumu verisini butonlara basmadan önce al (yoksa veri olmadığı için çalışmaz)
